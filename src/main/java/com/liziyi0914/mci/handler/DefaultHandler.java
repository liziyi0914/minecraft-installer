...
public class DefaultHandler implements Handler {
    ...
    public void genSubTasks(Cmd cmd, InstallContext ctx, TaskExecutor executor) {
        ...
        if (Objects.nonNull(cmd.getForge())) {
            executor.then(new VarTask<>(Identifiers.VAR_MIX, mix || Objects.nonNull(cmd.getLiteloader()) || Objects.nonNull(cmd.getOptifine())))
    ...
    public void handle(Cmd cmd, InstallContext ctx, TaskExecutor executor) {
        ctx.put(Identifiers.VAR_TASK_NAME, "安装 " + cmd.getId());
        ...
        if (Objects.nonNull(cmd.getLiteloader())) {
            ...
            ctx.put(Identifiers.VAR_LITE_LOADER_VERSION, cmd.getLiteloader());
        }
